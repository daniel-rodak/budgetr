      ...
      div(
        span(
          ...
          div(
            ...
            style = 'width:100%;',
            div(class = 'input-group-addon', icon('file')),
            tags$input(
...
#' Choose a File Interactively (Mac OS X)
#'
#' Display a file selection dialog under Mac OS X
#'
#' @param default which folder to show initially
#' @param caption the caption on the selection dialog
#'
#' @details
#' Uses an Apple Script to display a file selection dialog.  With \code{default = NA},
#' the initial folder selection is determined by default behavior of the
#' "choose folder" Apple Script command.  Otherwise, paths are expanded with
#' \link{path.expand}.
#'
...
directFileInput <- function(inputId, label, value = NULL) {
  directoryInput(inputId, label, value)
}

updateDirectFileInput = function(session, inputId, value = NULL, ...) {
  if (is.null(value)) {
    value = choose.file(...)
  }
  session$sendInputMessage(inputId, list(chosen_dir = value))
...
readDirectFileInput = function(session, inputId) {
  session$input[[sprintf('%s__chosen_dir', inputId)]]
}
